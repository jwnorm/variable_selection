...
The data set for this assignment was obtained from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/dataset/510/qsar+bioconcentration+classes+dataset). This one specifically is used for QSAR modeling. The following attributes are included:
...
To begin, let's fit a linear regression model with `logBCF` as the response and all other variable as the predictors.

```{r}
model1 <- lm(logBCF ~ ., data = train)
summary(model1)
```
...
Let's compute the AIC, BIC, and Mallow's $C_p$ values for the full model.
...
aic4 <- AIC(model4)
bic4 <- BIC(model4)
cp4 <- olsrr::ols_mallows_cp(model3, model4)
...
Using the above measures to evaluate the models, I would select `model4` as the preferred model. This is because all three models have similar AIC, BIC, and adjusted $R^2$ values, but the Mallow's $C_p$ value for `model4` is significantly lower than the other two.
...
We can see that *all* of the variables are selected. This is because ridge regression is not a variable selection technique, but rather is used for regularization. We can see that all estimated model coefficients are different from what we observed in our full model. This attempts to adjust for potential multicollinearity.
...
The optimal value of $\lambda$ that minimizes the cross validation error is $\lambda^*=$ `r round(lambda_opt_lasso, 4)`. What does the coefficient path look like as $\lambda$ varies?
...
Using elastic net, the same variables are selected as those found using LASSO regression.

### Question 7. Model Comparison

Let's bring everything together and test several model against our `test` tibble we defined earlier. Specifically, we will be comparing:
...
Using MSPE as our criterion, the backward stepwise regression model using BIC performed the best. The response variable is on a log scale so the MSPEs are all quite close.
...
The variables `nHM`, `piPC09`, `MLOGP`, and `F04[C-O]` were selected by every single method. Interestingly, `X2Av` was only selected by the full model and the ridge regression. These models do not exclude any variables by design, so we can be confident not to include this variable as a predictor in any model we build.
